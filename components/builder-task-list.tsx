    ...
    <Card>
      ...
      <div className="mt-5 space-y-3">
        {tasks.map((task, index) => (
          <div key={`${task.id}-${index}`} className="flex flex-col gap-3 rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-3.5 text-sm sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              <span className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 text-xs text-white/45">{String(index + 1).padStart(2, '0')}</span>
