  ...
  return (
    <>
      <button
        ...
        className="hidden items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-white/50 transition hover:bg-white/[0.06] lg:flex"
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 p-4 pt-20 backdrop-blur-sm" onClick={() => setOpen(false)}>
            ...
            <div className="max-h-[60vh] overflow-y-auto p-2">
              {results.map((item) => (
                <Link
                  ...
                  href={item.href}
                  className="flex items-center justify-between rounded-2xl px-4 py-3 text-sm text-white/70 transition hover:bg-white/5 hover:text-white"
                  onClick={() => setOpen(false)}
                >
                  <span>{item.title}</span>
                  <span className="text-xs uppercase tracking-[0.2em] text-white/30">{item.type}</span>
                </Link>
              ))}
